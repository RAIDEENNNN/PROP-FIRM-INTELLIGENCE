...
alter table public.prop_firm_rule_details
...
begin
  ...
  if duplicate_value is not null then
    raise exception 'Cannot add prop_firms lower(slug) unique index; duplicate slug exists: %', duplicate_value;
  end if;
  ...
  if duplicate_value is not null then
    raise exception 'Cannot add prop_firms lower(name) unique index; duplicate name exists: %', duplicate_value;
  end if;
end
$$;
...
set
  ...
  confidence_score = coalesce(confidence_score, trust_score)
where status = 'active'
...
update public.prop_firm_rule_details
set content_status = 'published'
where status = 'active'
  and content_status = 'draft';
...
create policy "Public reads published active prop firm rule details"
  on public.prop_firm_rule_details
  for select
  to anon, authenticated
  using (status = 'active' and content_status = 'published');
...
begin
  ...
  if duplicate_value is not null then
    raise exception 'Cannot add brokers lower(slug) unique index; duplicate slug exists: %', duplicate_value;
  end if;
  ...
  if duplicate_value is not null then
    raise exception 'Cannot add brokers lower(name) unique index; duplicate name exists: %', duplicate_value;
  end if;
end
$$;
