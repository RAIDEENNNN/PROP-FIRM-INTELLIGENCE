...
export const metadata: Metadata = {
  title: "Roadmap | FundedScope",
  description: "See what FundedScope has shipped, what is in progress, and what is coming next for trader intelligence."
};
...
const columns = [
  ...
  },
  {
    title: "In Progress",
    tone: "text-electric",
    items: ["Production API connection", "Supabase-backed profiles", "Verified reviews", "Rule change history", "Watchlists and alerts", "Broker spread feeds"]
  },
  {
    title: "Coming Soon",
    tone: "text-gold",
    items: ["Mobile app", "AI assistant", "Push notifications", "Trading journal memory", "Trader Connect", "Broker API access", "Premium reports"]
  }
];
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-10 sm:px-5 sm:py-12">
      <section className="rounded-[2rem] border border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(56,189,248,0.18),transparent_34%),rgba(255,255,255,0.03)] p-6 sm:p-8">
        <p className="text-sm uppercase tracking-[0.28em] text-electric">Product roadmap</p>
            ...
            <div className="mt-5 space-y-3">
              {column.items.map((item) => (
                <div key={item} className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 text-sm font-bold text-slate-200">
                  {column.title === "Completed" ? "☑" : column.title === "In Progress" ? "◐" : "☐"} {item}
                </div>
              ))}
            </div>
          </GlassCard>
      ...
      <GlassCard className="mt-8">
        ...
        <p className="mt-4 text-sm leading-7 text-slate-300">
          Features that do not improve discovery, comparison, tracking, verification, risk awareness or personal trading discipline stay lower on the roadmap.
        </p>
      </GlassCard>
    </main>
  );
