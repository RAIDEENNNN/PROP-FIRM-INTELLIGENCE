  ...
  return (
    <article className="glass glow-border group rounded-3xl p-5 transition duration-300 hover:-translate-y-1 hover:shadow-glow">
      ...
      <div className="mt-5 grid gap-2 sm:grid-cols-3">
        <Link href={`/prop-firms/${firm.slug}`} className="rounded-2xl bg-white px-4 py-3 text-center text-sm font-black text-void">
          View review
        </Link>
        <Link href={`/compare?firms=${firm.slug}`} className="rounded-2xl border border-white/10 px-4 py-3 text-center text-sm font-black text-white">
          Compare
        </Link>
        <a
          ...
          className="rounded-2xl border border-electric/30 px-4 py-3 text-center text-sm font-black text-electric"
        >
          Visit / code
        </a>
      </div>
      <p className="mt-3 text-xs leading-5 text-slate-500">Outbound links may be affiliate links. Scores remain editorial.</p>
    </article>
  );
}
