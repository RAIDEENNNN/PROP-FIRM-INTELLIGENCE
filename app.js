...
let userProfile = JSON.parse(localStorage.getItem('fundscope-profile') || 'null') || {
  ...
  savedFirmIds:['ftmo','fundingpips','niara-trader'],
  alertFirmIds:['ftmo','apex','niara-trader']
};
...
const state = { route:bootRoute.route || 'home', selectedFirm:bootRoute.firmId || 'ftmo', calc:'lot', compare:[0,1,3], search:'', filters:{ market:'all', status:'all', maxFee:'', minRating:'' }, accounts: JSON.parse(localStorage.getItem('fundscope-accounts') || 'null') || [
...
function bindFirmClicks(scope=document) {
  scope.querySelectorAll('[data-firm]').forEach(el=>el.onclick=()=>navigate('profile',el.dataset.firm));
}
...
function nav() {
  const links=[['home','Home'],['directory','Directory'],['compare','Compare'],['match','AI Match'],['calculators','Calculators'],['dashboard','Dashboard']];
  return `<nav class="nav"><div class="container nav-inner">
    <a class="brand" data-route="home">${brandLogo()} FundScope</a>
    <div class="nav-links">${links.map(([r,l])=>`<a class="nav-link ${state.route===r?'active':''}" data-route="${r}">${l}</a>`).join('')}<a class="nav-link ${state.route==='alerts'?'active':''}" data-route="alerts">Alerts</a><a class="nav-link ${state.route==='deals'?'active':''}" data-route="deals">Deals</a></div>
...
function footer() { return `<footer class="footer"><div class="container"><div class="footer-grid">
  ...
  <div><h4>Company</h4><a data-route="how-score">How we score</a><a data-route="editorial">Editorial policy</a><a data-route="contact">Contact</a></div>
...
function home() { const heroResults=filteredFirms().slice(0,5); const topPick=recommendedFirms()[0] || firms[0]; return `<main>
  <section class="hero hero-redesign"><div class="container hero-inner"><div><span class="eyebrow">AI-assisted prop firm intelligence</span><h1>Compare every serious prop firm before you buy a challenge.</h1><p class="hero-copy">Search rules, fees, payout terms, drawdown limits, discounts, rule changes, and trader-fit scores from one dashboard.</p><form class="search-box advanced-search" id="hero-search"><input aria-label="Search firms" id="search-input" value="${state.search}" placeholder="Search FTMO, Niara, futures, MT5, fast payouts..."/><button class="btn" type="submit">${icon('search')} <span>Search</span></button></form><div class="quick-filters"><button data-quick="forex">Forex</button><button data-quick="futures">Futures</button><button data-quick="crypto">Crypto</button><button data-route="match">Find my best match</button></div><div class="search-results" id="hero-results">${heroSearchMarkup(heroResults)}</div><div class="hero-meta"><span><b>${firms.length}+</b> firms tracked</span><span><b>${alerts.length}</b> policy alerts</span><span><b>${ruleChanges.length}</b> rule changes logged</span></div></div>
...
function infoPage(kind) {
  const pages = {
    ...
    contact: ['Contact', 'Use this page for partnerships, data corrections, feedback, or listing verification.', [['Email','hello@fundscope.local'],['Partnerships','Send firm name, official website, commission terms, approved logo assets, and current rule documents.'],['Corrections','Tell us the firm, field to update, source URL, and date checked.']]],
    ...
    privacy: ['Privacy policy', 'This MVP stores demo account preferences locally and in the JSON database when using the backend.', [['Collected data','Name, email, saved preferences, dashboard accounts, giveaway entries, feedback, and affiliate click events.'],['Use of data','Data is used to power recommendations, alerts, dashboards, and product feedback.'],['Production note','Before public launch, replace this MVP policy with a lawyer-reviewed privacy policy and real auth/database security.']]],
...
function bind() {
  ...
  const savePrefs=document.querySelector('#save-preferences'); if(savePrefs) savePrefs.onclick=async()=>{localStorage.setItem('fundscope-profile',JSON.stringify(userProfile)); if(currentUser){try{await api.post('/api/recommendations',{preferences:userProfile});await fetch(`/api/profiles/${currentUser.id}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(userProfile)});}catch{}} toast('Preferences saved and recommendations updated');};
  const authForm=document.querySelector('#auth-form'); if(authForm) authForm.onsubmit=async e=>{e.preventDefault();const d=new FormData(e.target);const mode=d.get('mode');try{const result=await api.post(`/api/auth/${mode}`,{name:d.get('name'),email:d.get('email'),password:d.get('password')});currentUser=result.user;localStorage.setItem('fundscope-user',JSON.stringify(currentUser));const profile=await api.get(`/api/profiles/${currentUser.id}`);if(profile){userProfile=profile;localStorage.setItem('fundscope-profile',JSON.stringify(userProfile));}toast('Signed in');render();}catch{toast('Could not sign in. Try demo@fundscope.local / demo123');}};
  const logout=document.querySelector('#logout'); if(logout) logout.onclick=()=>{currentUser=null;localStorage.removeItem('fundscope-user');toast('Logged out');render();};
...
async function loadBackendData(){
  try{
    ...
    if(currentUser){const savedProfile=profiles.find(p=>p.userId===currentUser.id);if(savedProfile) userProfile=savedProfile;}
