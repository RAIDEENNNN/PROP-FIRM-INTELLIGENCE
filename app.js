let firms = [
  ...
  { id:'fundednext', name:'FundedNext', initials:'FN', color:'#7b55c7', rating:4.4, reviews:812, ratingSource:'PropFirmMatch', fee:549, target:'8%', daily:'5%', drawdown:'10%', payout:'14 days', split:'95%', best:'Flexible challenges', founded:2022, platforms:'MT4, MT5, cTrader' },
  ...
  { id:'ftmo', name:'FTMO', initials:'FT', color:'#285fc2', rating:4.6, reviews:201, ratingSource:'PropFirmMatch', fee:540, target:'10%', daily:'5%', drawdown:'10%', payout:'14 days', split:'90%', best:'Proven track record', founded:2015, platforms:'MT4, MT5, cTrader' },
  { id:'the5ers', name:'The5ers', initials:'5%', color:'#d86a2d', rating:4.7, reviews:1222, ratingSource:'PropFirmMatch', fee:495, target:'8%', daily:'5%', drawdown:'10%', payout:'14 days', split:'100%', best:'Scaling plans', founded:2016, platforms:'MT5' },
  { id:'fundingpips', name:'Funding Pips', initials:'FP', color:'#151e29', rating:4.3, reviews:1112, ratingSource:'PropFirmMatch', fee:444, target:'8%', daily:'5%', drawdown:'10%', payout:'5 days', split:'100%', best:'Fast payouts', founded:2022, platforms:'MT5, Match-Trader' }
];
...
const brandLogo = () => `<span class="brand-mark" aria-hidden="true"><svg viewBox="0 0 48 48" fill="none"><path class="brand-shield" d="M24 4.5 39 10.7v12.4c0 9.5-6.1 17-15 20.4C15.1 40.1 9 32.6 9 23.1V10.7L24 4.5Z"/><circle cx="24" cy="23.5" r="12.2" stroke="white" stroke-opacity=".55" stroke-width="2.8"/><path d="M31.4 15.7 26.8 26.8 15.7 31.4l4.6-11.1 11.1-4.6Z" fill="#ffca5c" stroke="white" stroke-width="2.4" stroke-linejoin="round"/><path d="M24 10.4v4.2M24 32.4v4.2M10.9 23.5h4.2M32.9 23.5h4.2" stroke="white" stroke-width="2.6" stroke-linecap="round"/></svg></span>`;
...
function stars(f) {
  const hasRating = f.rating !== null && f.rating !== undefined && !Number.isNaN(Number(f.rating)) && Number(f.rating) > 0;
  if (!hasRating && f.ratingLabel) {
    const label = esc(f.ratingLabel);
    const sourceLabel = f.ratingSource ? ` · ${esc(f.ratingSource)}` : '';
    return `<span class="stars" aria-label="${label}"><small>${label}${sourceLabel}</small></span>`;
  }
  const rating = Number(f.rating || 0).toFixed(1);
  const reviewsCount = Number(f.reviews || 0);
  const reviewsText = reviewsCount > 0 ? ` (${reviewsCount.toLocaleString()})` : (f.reviewsLabel ? ` (${esc(f.reviewsLabel)})` : '');
  const source = f.ratingSource ? ` · ${esc(f.ratingSource)}` : ' · public rating estimate';
  return `<span class="stars" aria-label="${rating} out of 5">${starText(f.rating)} <small>${rating}${reviewsText}${source}</small></span>`;
}
